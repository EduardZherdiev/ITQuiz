...
def build_bootstrap_payload(session: Session, user_id: str, language: str = LANGUAGE_CODE) -> dict:
    ...
    return {
        ...
        "topic_texts": session.query(TopicText).filter(TopicText.language_code.in_(("en", "ru", "uk"))).order_by(TopicText.topic_id, TopicText.language_code).all(),
        "questions": session.query(Question).filter(Question.is_active.is_(True)).order_by(Question.id).all(),
        "question_texts": session.query(QuestionText).filter(QuestionText.language_code.in_(("en", "ru", "uk"))).order_by(QuestionText.question_id, QuestionText.language_code).all(),
        "options": session.query(Option).order_by(Option.id).all(),
        "option_texts": session.query(OptionText).filter(OptionText.language_code.in_(("en", "ru", "uk"))).order_by(OptionText.option_id, OptionText.language_code).all(),
